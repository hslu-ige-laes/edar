...
The book is structured in the following parts main parts:
...
The part ``R Basics'' covers general data analysis tasks like data loading, wrangling and aggregation. Mostly this part deals with number juggling and table viewing. An ``R-beginner'' receives information about the installation of the program environment and gets a quick practical introduction.

\begin{quote}
``Numerical quantities focus on expected values, graphical summaries on unexpected values.'' - John W. Tukey
\end{quote}

Visualizations are good to identify patterns, changes over time, unusual readings, and relationships between variables. Examples for such graphical representations are covered in the part ``Data Visualizations'' which brings the calculated numbers to life. The presented code makes the creation of common and useful plots for energy and comfort data fast and easy.
...
Before we can start with the analysis described in the later chapters, we have to installed ``R''.
...
If you already have ``R'' and ``R Studio'' installed, please take a look at the hslu-ige-laes/redutils package on github, which is frequently used throughout this book. Chapter \ref{installationPackages} shows you how to install it.

Finally beginners find in \ref{createFirstScript} an easy code example which creates a simple time series plot.
...
This is a great resource where you can find everything you need to know about creating basic plots. Therefore this topic is not covered again in this book.
...
However, this plot is only useful if the period of the seasonality is already known. In many cases, this will in fact be known. For example, monthly data typically has a period of 12. If the period is not known, an autocorrelation plot or spectral plot can be used to determine it.
...
Create a plot of detrended mean data per week.
...
Finally, some visualizations that did not fit into any of the above chapters and are not worth a separate chapter. But these are not less interesting\ldots{}
