...
\item
  ...
  11 timestamp of reading values Nr. 1\ldots13
  ...
  (in case of a failure this value does not get increased)
  ...
  20 M-Bus Stateogrammed planned reading
  20 M-Bus State
\end{itemize}
...
\CommentTok{# add metadata for later filtering}
\NormalTok{df}\OperatorTok{$}\NormalTok{timestamp <-}\StringTok{ }\KeywordTok{parse_date_time}\NormalTok{(df}\OperatorTok{$}\NormalTok{timestamp, }\StringTok{"YmdHMS"}\NormalTok{, }\DataTypeTok{tz =} \StringTok{"Europe/Zurich"}\NormalTok{)}
\NormalTok{df}\OperatorTok{$}\NormalTok{year <-}\StringTok{ }\KeywordTok{year}\NormalTok{(df}\OperatorTok{$}\NormalTok{timestamp)}
\NormalTok{df}\OperatorTok{$}\NormalTok{month <-}\StringTok{ }\KeywordTok{month}\NormalTok{(df}\OperatorTok{$}\NormalTok{timestamp)}

\CommentTok{# date to differentiate before/after}
\NormalTok{dateOptimization <-}\StringTok{ "2017-09-01"}

\CommentTok{# create two separate data frames for later visualization (before is grey, after is colored)}
\NormalTok{df.after<-}\StringTok{ }\NormalTok{df }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(timestamp }\OperatorTok{>=}\StringTok{ }\NormalTok{dateOptimization) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{na.omit}\NormalTok{()}
\NormalTok{df.before <-}\StringTok{ }\NormalTok{df }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(timestamp }\OperatorTok{<=}\StringTok{ }\NormalTok{dateOptimization) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{na.omit}\NormalTok{()}

\CommentTok{# add statistical band values and median of phase "before"}
\NormalTok{df.before <-}\StringTok{ }\NormalTok{df.before }\OperatorTok{%>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(month) }\OperatorTok{%>%}\StringTok{ }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{median =} \KeywordTok{quantile}\NormalTok{(value, }\FloatTok{0.5}\NormalTok{),}
                                                             \DataTypeTok{qLower =} \KeywordTok{quantile}\NormalTok{(value, }\FloatTok{0.05}\NormalTok{),}
                                                             \DataTypeTok{qUpper =} \KeywordTok{quantile}\NormalTok{(value, }\FloatTok{0.95}\NormalTok{)}
\NormalTok{                                                             ) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{ungroup}\NormalTok{()}

\CommentTok{# calculate values to later generate different colors}
\NormalTok{count.all <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(df }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(year) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unique}\NormalTok{())}
\NormalTok{count.after <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(df.after }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(year) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unique}\NormalTok{())}
\NormalTok{count.baseline <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(df.before }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(year) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unique}\NormalTok{())}
\CommentTok{# correction in case the date is during a year}
\ControlFlowTok{if}\NormalTok{((count.after}\OperatorTok{+}\NormalTok{count.baseline) }\OperatorTok{>}\StringTok{ }\NormalTok{count.all)\{count.baseline <-}\StringTok{ }\NormalTok{count.baseline }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{\}}

\CommentTok{# plot graph with all time series}
\NormalTok{g <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ df.before, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ month, }\DataTypeTok{y =}\NormalTok{ value, }\DataTypeTok{group =}\NormalTok{ year, }\DataTypeTok{color =} \KeywordTok{factor}\NormalTok{(year)), }\DataTypeTok{alpha =} \FloatTok{0.5}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_ribbon}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ df.before, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ month, }\DataTypeTok{ymin =}\NormalTok{ qLower, }\DataTypeTok{ymax =}\NormalTok{ qUpper), }\DataTypeTok{fill =} \StringTok{"orange"}\NormalTok{, }\DataTypeTok{alpha =} \FloatTok{0.3}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ df.before, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ month, }\DataTypeTok{y =}\NormalTok{ median), }\DataTypeTok{colour =} \StringTok{"orange"}\NormalTok{, }\DataTypeTok{linetype =} \StringTok{"dashed"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ df.after, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ month, }\DataTypeTok{y =}\NormalTok{ value, }\DataTypeTok{group =}\NormalTok{ year, }\DataTypeTok{colour =} \KeywordTok{factor}\NormalTok{(year)), }\DataTypeTok{alpha =} \FloatTok{0.8}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ df.after, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ month, }\DataTypeTok{y =}\NormalTok{ value, }\DataTypeTok{group =}\NormalTok{ year, }\DataTypeTok{color =} \KeywordTok{factor}\NormalTok{(year)), }\DataTypeTok{alpha =} \FloatTok{0.8}\NormalTok{, }\DataTypeTok{shape =} \DecValTok{21}\NormalTok{, }\DataTypeTok{fill =} \StringTok{"white"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_economist}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Before/After Optimization}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{, }\DataTypeTok{x =} \StringTok{"Month"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Energy Consumption }\CharTok{\textbackslash{}n}\StringTok{(kWh/month)"}\NormalTok{, }\DataTypeTok{color =} \StringTok{"Years}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_x_discrete}\NormalTok{(}\DataTypeTok{limits =}\NormalTok{ month.abb) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_manual}\NormalTok{(}\DataTypeTok{values=}\KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"grey"}\NormalTok{, count.baseline), viridis}\OperatorTok{::}\KeywordTok{viridis}\NormalTok{(count.after)))}
  
\KeywordTok{ggplotly}\NormalTok{(g)}
\end{Highlighting}
\end{Shaded}

\includegraphics{edar_files/figure-latex/energyConsBeforeAfter-1.pdf}
