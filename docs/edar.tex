...
A really good source is R for Data Science by Garrett Grolemund and Hadley Wickham. The entire book is freely available online through the same format of the coursebook.
...
\item
  Copy the following code and paste it into your console right of the symbol \texttt{\textgreater{}}:
\end{itemize}
...
\hypertarget{create-your-first-r-script-and-visualization}{%
\section{Create your first R Script and visualization}\label{create-your-first-r-script-and-visualization}}
...
\begin{Highlighting}[]
\NormalTok{redutils}\OperatorTok{::}\KeywordTok{season}\NormalTok{(}\KeywordTok{as.Date}\NormalTok{(}\StringTok{"2019-04-01"}\NormalTok{),}
                 \KeywordTok{c}\NormalTok{(}\StringTok{"Winter"}\NormalTok{,}\StringTok{"Frühling","}\NormalTok{Sommer}\StringTok{","}\NormalTok{Herbst}\StringTok{"))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{df <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://github.com/hslu-ige-laes/edar/raw/master/sampleData/heatPumpElectricity.csv"}\NormalTok{,}
               \DataTypeTok{stringsAsFactors=}\OtherTok{FALSE}\NormalTok{,}
               \DataTypeTok{sep =}\StringTok{";"}\NormalTok{)}
...
\CommentTok{# filter time range if necessary}
\NormalTok{df <-}\StringTok{ }\NormalTok{df }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(timestamp }\OperatorTok{>}\StringTok{ "2019-11-01 00:00:00"}\NormalTok{, timestamp }\OperatorTok{<}\StringTok{ "2020-04-01 00:00:00"}\NormalTok{)}

\CommentTok{# Fill missing values with NA}
\NormalTok{grid.df <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{timestamp =} \KeywordTok{seq}\NormalTok{(df[}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{], df[}\KeywordTok{nrow}\NormalTok{(df), }\DecValTok{1}\NormalTok{], }\DataTypeTok{by =} \StringTok{"15 mins"}\NormalTok{))}
\NormalTok{df <-}\StringTok{ }\KeywordTok{merge}\NormalTok{(df, grid.df, }\DataTypeTok{all =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# convert steadily counting energy meter value in kWh to power in Watts }
\NormalTok{df <-}\StringTok{ }\NormalTok{df }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{value =}\NormalTok{ (meterValue }\OperatorTok{-}\StringTok{ }\KeywordTok{lag}\NormalTok{(meterValue))}\OperatorTok{/}\DecValTok{4}\OperatorTok{*}\DecValTok{1000}\NormalTok{) }\OperatorTok{%>%}
...
\CommentTok{# df$time <- format(df$timestamp, "%H:%M")}
\NormalTok{df}\OperatorTok{$}\NormalTok{time <-}\StringTok{ }\KeywordTok{format}\NormalTok{(}\KeywordTok{as.POSIXct}\NormalTok{(}\KeywordTok{strptime}\NormalTok{(df}\OperatorTok{$}\NormalTok{timestamp, }\StringTok{"%Y-%m-%d  %H:%M:%S"}\NormalTok{,}\DataTypeTok{tz=}\StringTok{""}\NormalTok{)) ,}\DataTypeTok{format =} \StringTok{"%H:%M"}\NormalTok{)}
\NormalTok{df}\OperatorTok{$}\NormalTok{time <-}\StringTok{ }\KeywordTok{as.POSIXct}\NormalTok{(df}\OperatorTok{$}\NormalTok{time, }\DataTypeTok{format =} \StringTok{"%H:%M"}\NormalTok{)}
...
\CommentTok{# df$day <- as.Date(cut(df$timestamp, breaks = "day"))}
...
\StringTok{  }\KeywordTok{plot_ly}\NormalTok{(}\DataTypeTok{x=}\OperatorTok{~}\NormalTok{x,}
          ...
          \DataTypeTok{hovertemplate =} \KeywordTok{paste}\NormalTok{(}\StringTok{"Time: "}\NormalTok{, }\KeywordTok{format}\NormalTok{(df}\OperatorTok{$}\NormalTok{time, }\StringTok{"%H:%M"}\NormalTok{),}
                                \StringTok{"<br>Date: "}\NormalTok{, }\KeywordTok{format}\NormalTok{(df}\OperatorTok{$}\NormalTok{time, }\StringTok{"%Y-%m-%d"}\NormalTok{),}
                                \StringTok{"<br>Mean: %\{y:.0f\}"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{  }
...
\NormalTok{           ),}
           \DataTypeTok{yaxis =} \KeywordTok{list}\NormalTok{(}
             \DataTypeTok{title =} \StringTok{"Power (W)"}\NormalTok{,}
             \DataTypeTok{range =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(df}\OperatorTok{$}\NormalTok{value)) }
\NormalTok{           )}
...
\StringTok{  }\KeywordTok{highlight}\NormalTok{(}\DataTypeTok{on =} \StringTok{"plotly_hover"}\NormalTok{,}
            ...
            \DataTypeTok{opacityDim =} \FloatTok{1.0}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
\includegraphics{edar_files/figure-latex/energyprofiles plotly-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate Mean value for all 15 minutes for each weekday}
\NormalTok{df <-}\StringTok{ }\NormalTok{df }\OperatorTok{%>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(weekday, x) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{dayTimeMean =} \KeywordTok{mean}\NormalTok{(value)) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{ungroup}\NormalTok{()}

\CommentTok{# shrink data frame}
\NormalTok{df <-}\StringTok{ }\NormalTok{df }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(x, weekday, time, dayTimeMean) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{unique}\NormalTok{() }\OperatorTok{%>%}
...
\NormalTok{         ),}
         \DataTypeTok{yaxis =} \KeywordTok{list}\NormalTok{(}
             \DataTypeTok{title =} \StringTok{"Power (W)"}\NormalTok{,}
             \DataTypeTok{range =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(df}\OperatorTok{$}\NormalTok{dayTimeMean))  }
\NormalTok{         )}
\NormalTok{  ) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{highlight}\NormalTok{(}\DataTypeTok{on =} \StringTok{"plotly_hover"}\NormalTok{,}
            \DataTypeTok{off =} \StringTok{"plotly_doubleclick"}\NormalTok{,}
            \DataTypeTok{color =} \StringTok{"orange"}\NormalTok{,}
            \DataTypeTok{opacityDim =} \FloatTok{0.7}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{plotly}\OperatorTok{::}\KeywordTok{config}\NormalTok{(}\DataTypeTok{modeBarButtons =} \KeywordTok{list}\NormalTok{(}\KeywordTok{list}\NormalTok{(}\StringTok{"toImage"}\NormalTok{)), }\DataTypeTok{displaylogo =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{edar_files/figure-latex/energyprofiles plotly-2.pdf}
