...
\NormalTok{data <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://github.com/hslu-ige-laes/edar/raw/master/sampleData/centralOutsideTemp.csv"}\NormalTok{,}
...
\NormalTok{df <-}\StringTok{ }\NormalTok{data}
...
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(df,}\DecValTok{2}\NormalTok{)}
\CommentTok{##                  time centralOutsideTemp season}
\CommentTok{## 1 2018-03-21 11:00:00                5.2 Spring}
\CommentTok{## 2 2018-03-21 12:00:00                6.7 Spring}
\KeywordTok{tail}\NormalTok{(df,}\DecValTok{2}\NormalTok{)}
\CommentTok{##                      time centralOutsideTemp season}
\CommentTok{## 21864 2020-09-17 10:00:00              26.65   Fall}
\CommentTok{## 21865 2020-09-17 11:00:00              28.10   Fall}
\end{Highlighting}
\end{Shaded}
...
\subsection{Merge two}\label{DATAWRANGLING-MERGE-TWO-DATAFRAMES}}
...
Create a mollier h-x diagram with
- scatter plot of temperature- and humidity sensor data (mean values per day)
- points colored according to season
- comfort zone

The sensor data is not in a constant intervall and not yet aggregated.

\hypertarget{solution-1}{%
\subsection{Solution}\label{solution-1}}

Use the plot function \texttt{mollierHxDiagram} from the \texttt{redutils} package (R Energy Data Utilities) which you can install as followed:
...
Afterwards create a new script, copy/paste the following code and run it:
